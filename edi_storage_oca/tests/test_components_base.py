...
class EDIStorageComponentTestCase(TestEDIStorageBase):
    def test_remote_file_path(self):
        ...
        for _args, expected in to_test:
            path_obj = self.checker._make_remote_file_path(*_args)
            self.assertEqual(path_obj.as_posix(), expected)

        with self.assertRaises(AssertionError):
            self.checker._make_remote_file_path("WHATEVER", "error", "foo.csv")

        with self.assertRaises(AssertionError):
            self.checker._make_remote_file_path("input", "WHATEVER", "foo.csv")
