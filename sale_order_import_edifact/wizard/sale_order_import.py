...
class SaleOrderImport(models.TransientModel):
    ...
    edifact_ok = fields.Boolean(compute="_compute_edifact_ok", store=True)

    @api.depends("order_file", "order_filename")
    def _compute_edifact_ok(self):
        for rec in self:
            if rec.order_file and rec.order_filename:
    ...
    def order_file_change(self):
        # Support get context if using EDI machinery
        res = super().order_file_change()
        self.price_source = self.env.context.get("price_source")
        return res
    ...
    def parse_edifact_order(self, filecontent, detect_doc_type=False):
        ...
        if detect_doc_type:
            if not self.edifact_ok:
                return None
            return self.env.context.get("doc_type", "rfq")
