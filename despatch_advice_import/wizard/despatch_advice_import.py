...
class DespatchAdviceImport(models.TransientModel):
    ...
    allow_validate_over_qty = fields.Boolean("Allow Validate Over Quantity")
    ...
    def process_data(self, parsed_order_document):
        ...
        for line in lines:
            ...
            allow_validate_over_qty = self.allow_validate_over_qty or self._context.get(
                "allow_validate_over_qty", False
            )
            if line_info["qty"] == moves_qty:
                self._process_accepted(stock_moves, parsed_order_document)
            elif line_info["qty"] > moves_qty and allow_validate_over_qty:
