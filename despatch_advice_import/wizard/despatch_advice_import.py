...
class DespatchAdviceImport(models.TransientModel):
    ...
    def get_xml_files_from_pdf(self, document: bytes):
        ...
        """
        xml_files = {}
        reader = OdooPdfFileReader(BytesIO(document), strict=False)
        for filename, content in reader.getAttachments():
            filetype = mimetypes.guess_type(filename)[0]
            if filetype not in {"application/xml", "text/xml"}:
                continue
            try:
                xml_files[filename] = etree.fromstring(content)
            except Exception:
                logger.info(
                    "Skipping embedded XML attachment %s: invalid XML", filename
                )
        return xml_files
    ...
    def _process_accepted(
        ...
        """Mark moves as delivered, optionally with an over-delivered quantity.

        :param stock_moves: The open stock moves linked to the purchase order line.
        :param parsed_order_document: The normalized despatch advice data being applied.
        :param forced_qty: The delivered quantity to force on the first move, if any.
        """
        ...
        if forced_qty:
            stock_moves[0]._set_quantity_done(forced_qty)
            stock_moves[0].picked = True
            for move in stock_moves[1:]:
    ...
    def _process_conditional(
        ...
        """
        precision = moves[:1].product_uom.rounding
        ...
        if float_compare(qty, moves_qty, precision_rounding=precision) >= 0:
            raise UserError(
                self.env._(
        ...
        for move in moves:
            if (
                float_compare(qty, move.product_uom_qty, precision_rounding=precision)
                >= 0
            ):
            ...
            if (
                qty
                and float_compare(
                    qty, move.product_uom_qty, precision_rounding=precision
                )
                < 0
            ):
            ...
            if (
                float_compare(
                    backorder_qty, move.product_uom_qty, precision_rounding=precision
                )
                < 0
            ):
