...
class PurchaseOrderImport(models.TransientModel):
    ...
    def _get_purchase_id(self) -> models.Model:
        ...
        if self.env.context.get("active_model") != "purchase.order":
            raise UserError(
                self.env._("This wizard should be called from a purchase order.")
            )
    ...
    def _create_missing_order_lines(
        ...
        if compare_res["to_add"]:
            ...
            for add in compare_res["to_add"]:
                line_vals = self._prepare_order_line_import_vals(
    ...
    @api.model
    def _prepare_order_line_import_vals(
        self,
        product: models.Model,
        uom: models.Model,
        import_line: dict[str, Any],
    ) -> dict[str, Any]:
        """Prepare a order line import vals for a quoted product missing on the RFQ."""
        return {
