...
class BaseWamasUbl(models.AbstractModel):
    ...
    @api.model
    def record_to_wamas(self, specific_dict, telegram_type):
        """
        Export data as WAMAS format
        ...
        if not isinstance(specific_dict, dict):
            raise ValueError(_("The specific dict is not valid."))
        return self.dict2wamas(specific_dict, telegram_type)
