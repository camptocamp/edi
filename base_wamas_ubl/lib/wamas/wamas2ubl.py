...
class Extractor:
    ...
    def get_line(self, telegram_type, transfer_key_name, package_key_name=False):
        ...
        Parameters:
            telegram_type: the key to get the list out of data
            transfer_key_name: the key in the dict that serves to identify the parent in transfers
            package_key_name: the key in the dict that serves to identify the related package
        """
...
def wamas2ubl(wamas_msg, extra_data=False):
    ...
    msg_type = utils.detect_wamas_type(wamas_msg)[0]
