...
def wamas2dict(infile, verbose=False):
    ...
    for line in infile.split("\n"):
        ...
        grammar = globals()[telegram_type.lower()].grammar
...
def wamas2ubl(infile, verbose=False, extra_data=False):
    ...
    tmpl_path = False
    if top_keys in DICT_TUPLE_KEY_RECEPTION.keys():
        template_type = "reception"
        tmpl_path = DICT_TUPLE_KEY_RECEPTION[top_keys][0]
        order_name = DICT_TUPLE_KEY_RECEPTION[top_keys][1][0]
        order_key = DICT_TUPLE_KEY_RECEPTION[top_keys][1][1]
        line_name = DICT_TUPLE_KEY_RECEPTION[top_keys][2][0]
        line_key = DICT_TUPLE_KEY_RECEPTION[top_keys][2][1]
        receptions = _prepare_receptions(
            data, order_name, order_key, line_name, line_key
        )
    elif top_keys in DICT_TUPLE_KEY_PICKING.keys():
        template_type = "picking"
        tmpl_path = DICT_TUPLE_KEY_PICKING[top_keys]
    ...
    # 3) get template
    ubl_template = file_open(file_path(tmpl_path)).read()  # noqa: F405
