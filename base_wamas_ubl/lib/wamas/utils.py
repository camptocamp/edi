...
def fw2dict(line, grammar, telegram_type):
    ...
    if not min_length <= len(line) <= max_length:
        raise Exception(
            "Line of length {actual:d} does not match expected length of {expected:d}:\n{line:s}".format(
                actual=len(line),
                expected=max_length,
                line=line,
            )
...
def detect_wamas_type(infile):
    ...
    Returns:
        str: Type of message
        list: List of telegram types
    """
    lst_telegram_type = [
        get_telegram_type(line) for line in infile.strip("\n").splitlines()
    ]
    wamas_type = DICT_DETECT_WAMAS_TYPE.get(infile[40:43], "Undefined")
    return wamas_type, lst_telegram_type
